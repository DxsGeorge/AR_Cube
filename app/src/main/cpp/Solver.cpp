...
string Solver::Solve(RubiksCube& scrambledCube)
{
    int iteration = 1;
    ...
    do
    {
        ...
        iteration++;
    } while (result == NOT_FOUND && (solved == false));
...
int Solver::Search2(int cornerPermutation, int nonMiddleSliceEdgePermutation, int middleSliceEdgePermutation, int depth) {
    ...
    if (cost == 0) {	// Solution found...
        ...
        PrintSolution();
		if (solutionLength1+solutionLength2 <= 20) solved = true;
        return FOUND;
    }
...
void Solver::PrintSolution(void) {
	...
	for (int i = 0; i < solutionLength1; i++)
	{
		...
		solution.append(Cube::NameOfMove(TranslateMove(solutionMoves1[i], solutionPowers1[i], 0)));
		solution.append(" ");
	}
	...
	for (int i = 0; i < solutionLength2; i++)
	{
		...
		solution.append(Cube::NameOfMove(TranslateMove(solutionMoves2[i], solutionPowers2[i], 1)));
		solution.append(" ");
	}
